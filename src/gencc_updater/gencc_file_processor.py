...
def trim_line(splitted_line):
    ...
    for splitted in splitted_line:
        new_splitted = unicodedata.normalize(
            "NFKD", splitted.strip(" ").replace("\t", "").replace('"', "")
        )
        ...
        # new_splitted_line.append(new_splitted)
...
def read_gencc_file(gencc_raw_file: str, gencc_keys: list) -> dict:
    ...
    Args:
        ...
        gencc_keys (list): 리턴할 dictionary에서 value에 들어갈 data에 대한 column들의 리스트
